...
def solve_allocation(carton_types, shipping_items):
    ...
    if pulp.LpStatus[prob.status] != "Optimal":
        return None, "Tidak ada solusi optimal. Cek konsistensi data (CBM & jumlah karton)."
...
def distribute_results(solved_counts, shipping_items, carton_types):
    ...
    items = sorted(shipping_items, key=lambda r: (r['preference'] == '' or r['preference'] is None, r['count']))
...
carton_editor = [
    [
        sg.Text('Nama'), sg.Input(key='-CT_NAME-', size=(14,1)),
        sg.Text('P'),    sg.Input(key='-CT_L-', size=(6,1)),
        sg.Text('L'),    sg.Input(key='-CT_W-', size=(6,1)),
        sg.Text('T'),    sg.Input(key='-CT_H-', size=(6,1)),
        sg.Button('Tambah', key='-ADD_CT-')
    ],
    [sg.Table(values=[], headings=['Nama', 'P(cm)', 'L(cm)', 'T(cm)', 'Vol (m³)'],
              key='-CT_TABLE-', auto_size_columns=False,
              col_widths=[14,8,8,8,10], justification='right',
              display_row_numbers=True, num_rows=5, enable_events=True)],
    [sg.Button('Hapus Terpilih', key='-DEL_CT-'), sg.Button('Reset Tipe', key='-RESET_CT-')]
]

# Bagian item pengiriman
item_editor = [
    [
        sg.Text('Item'),   sg.Input(key='-ITEM_NAME-', size=(14,1)),
        sg.Text('Jml'),    sg.Input(key='-ITEM_COUNT-', size=(6,1)),
        sg.Text('CBM'),    sg.Input(key='-ITEM_CBM-', size=(8,1)),
        sg.Text('Preferensi'),
        sg.Combo(values=[], key='-ITEM_PREF-', size=(14,1), readonly=True),
        sg.Button('Tambah', key='-ADD_ITEM-')
    ],
    [sg.Table(values=[], headings=['Item', 'Jml Karton', 'Total CBM', 'Preferensi'],
              key='-ITEM_TABLE-', auto_size_columns=False,
              col_widths=[14,10,10,14], justification='right',
              display_row_numbers=True, num_rows=7, enable_events=True)],
    [sg.Button('Hapus Terpilih', key='-DEL_ITEM-'), sg.Button('Reset Item', key='-RESET_ITEM-')]
]
...
layout = [
    ...
    [sg.Button('HITUNG & ALOKASIKAN', key='-CALC-', size=(28,2), font=('Helvetica', 12, 'bold'))],
    [sg.Multiline(key='-OUT-', size=(90,18), font=('Consolas', 10), autoscroll=True, disabled=True)]
]
...
def refresh_ct_table(win):
    win['-CT_TABLE-'].update(values=[
        [d['name'], d['l'], d['w'], d['h'], f"{d['volume']:.5f}"] for d in carton_types
    ])
    win['-ITEM_PREF-'].update(values=[d['name'] for d in carton_types])

def refresh_item_table(win):
    win['-ITEM_TABLE-'].update(values=[
...
while True:
    ...
    if event == '-ADD_CT-':
        try:
            ...
            l = float(v['-CT_L-']); w = float(v['-CT_W-']); h = float(v['-CT_H-'])
    ...
    if event == '-ADD_ITEM-':
        try:
            ...
            if not name or count <= 0 or cbm <= 0:
                raise ValueError
            shipping_items.append({'name': name, 'count': count, 'cbm': cbm, 'preference': pref})
    ...
    if event == '-CALC-':
        ...
        lines.append(f"Total CBM (target) = {sum(i['cbm'] for i in shipping_items):.5f} m³")
        total_vol = sum(solved[n]*next(ct['volume'] for ct in carton_types if ct['name']==n) for n in solved)
