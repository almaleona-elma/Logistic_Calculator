document.addEventListener("DOMContentLoaded", () => {
  ...
  function showDusModal(templatesList, allocMap) {
    dusSel.innerHTML = "";
  ...
  // ── Modal Transfer (pindah karton antar item) ──
  const xfOverlay = $("modal-transfer-overlay"),
    xfQtyEl = $("transfer-qty"),
    xfToEl = $("transfer-to"),
    xfPrev = $("transfer-preview");
  let xfResolver = null,
    xfCtx = null; // xfCtx = { fromIdx, ci, tplId, maxQty, tp }

  function showTransferModal(fromIdx, ci) {
    ...
    $("transfer-karton-label").textContent =
      `${tp.name} (${tp.p}×${tp.l}×${tp.t}) — ${c.qty} krt`;
    xfQtyEl.value = 1;
    xfQtyEl.max = c.qty;
  ...
  function updateTransferPreview() {
    ...
    const cbmPU =
      xfCtx.tp.qtyTotal > 0
        ? xfCtx.tp.cbmTarget / xfCtx.tp.qtyTotal
        : xfCtx.tp.cbmPerUnit;
  ...
  $("transfer-ok").addEventListener("click", () => {
    ...
    // Kurangi dari item asal
    const srcCarton = items[xfCtx.fromIdx].cartons[xfCtx.ci];
    srcCarton.qty -= qty;
    if (srcCarton.qty <= 0) items[xfCtx.fromIdx].cartons.splice(xfCtx.ci, 1);

    // Tambah ke item tujuan (merge jika tplId sama)
    const dest = items[toIdx];
    const existing = dest.cartons.find((c) => c.tplId === xfCtx.tplId);
    if (existing) existing.qty += qty;
    else dest.cartons.push({ tplId: xfCtx.tplId, qty });
  ...
  function buildAllocMap() {
    const m = new Map();
    items.forEach((it) =>
      it.cartons.forEach((c) => m.set(c.tplId, (m.get(c.tplId) || 0) + c.qty)),
    );
    return m;
  }
  ...
  /** Calculate CFR/FOB for all items. Remainder FOB goes to smallest item (overproduction). */
  function calcAllCfrFob(freights) {
    const globalFob = pf(gFob.value);
    // Per-item base calculation
    const results = items.map((item, idx) => {
      const base = getItemCfrFob(item, freights[idx]);
      return { cfr: base.cfr, fob: base.fob, freight: freights[idx] };
    });
    // Remainder logic: need 2+ items, all with CFR/FOB, and global FOB
    const allHaveValues = items.length >= 2 && results.every((r) => r.cfr > 0);
    if (!allHaveValues || globalFob <= 0) return results;
    // Find smallest item by CFR (biasanya item overproduction)
    let sIdx = 0,
      sCfr = Infinity;
    results.forEach((r, i) => {
      if (r.cfr < sCfr) {
    ...
    // Sum FOB of non-smallest items
    let sumFob = 0;
    results.forEach((r, i) => {
      if (i !== sIdx) sumFob += r.fob;
    });
    // Remainder FOB & adjusted freight for smallest item
  ...
  function updateGlobal() {
    ...
    if (hasFr && hasCfr && gLastEdited !== "fob") {
      ...
      gHint.textContent =
        "Isi 2 dari 3: Freight, CFR, FOB — yang ke-3 otomatis terhitung";
      gHint.style.color = "";
    }
  }

  gFreight.addEventListener("input", () => {
    gLastEdited = "freight";
    updateGlobal();
    renderAll();
  });
  gCfr.addEventListener("input", () => {
    gLastEdited = "cfr";
    updateGlobal();
    renderAll();
  });
  gFob.addEventListener("input", () => {
    gLastEdited = "fob";
    updateGlobal();
    renderAll();
  });
  ...
  document
    .querySelectorAll('.paste-zone input[type="file"]')
    .forEach((input) => {
  ...
  async function processZonePdf(zone, file) {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        setBar(
          zone,
          10 + Math.round((i / pdf.numPages) * 80),
          `Halaman ${i}/${pdf.numPages}`,
        );
      }
  ...
  function parseMeasurement(text) {
    ...
    for (const line of text.split("\n")) {
      let m = line.match(
        /(\d+[.,]\d+)\s*(?:M3|CBM|m3)\s*.*?(\d+[.,]?\d*)\s*[xX×*]\s*(\d+[.,]?\d*)\s*[xX×*]\s*(\d+[.,]?\d*)/i,
      );
      if (!m) {
        m = line.match(
          /(\d+[.,]?\d*)\s*[xX×*]\s*(\d+[.,]?\d*)\s*[xX×*]\s*(\d+[.,]?\d*)\s*.*?(\d+[.,]\d+)\s*(?:M3|CBM)/i,
        );
        if (m) m = [null, m[4], m[1], m[2], m[3]];
      }
      if (m) {
        const cbm = pf(m[1].replace(",", ".")),
          p = pf(m[2].replace(",", ".")),
          l = pf(m[3].replace(",", ".")),
          t = pf(m[4].replace(",", "."));
        if (cbm > 0 && p > 0 && l > 0 && t > 0)
          parsed.measurement.push({ p, l, t, cbm });
      }
    }
    ...
    if (!parsed.measurement.length) {
      tb.innerHTML =
        '<tr><td colspan="5" class="empty">Tidak terdeteksi. Pastikan format "P x L x T" & "M3/CBM".</td></tr>';
      return;
    }
    tb.innerHTML = "";
    ...
    const bindOcr = (attr, key, parse = pf) =>
      tb.querySelectorAll(`[${attr}]`).forEach((el) =>
        el.addEventListener("change", (e) => {
          parsed.measurement[+e.target.dataset[key]][
            { mp: "p", ml: "l", mt: "t", mc: "cbm" }[key]
  ...
  function parseItems(text) {
    ...
    for (const line of text.split("\n")) {
      const m = line.match(
        /ITEM\s*(?:NO\.?|#)?\s*(\d+)\s*[:\-=]?\s*(\d+[.,]?\d*)\s*/i,
      );
      if (m)
        parsed.items.push({
          itemNo: pi(m[1]),
          qty: Math.round(pf(m[2].replace(",", "."))),
        });
    }
    const totalMatch = text.match(
      /NUMBER\s*OF\s*CARTON[S]?\s*[:\-=]?\s*(\d+[.,]?\d*)/i,
    );
    ...
    if (!parsed.items.length) {
      pre.innerHTML =
        '<div class="empty" style="padding:8px;font-size:12px">Tidak terdeteksi. Format: "ITEM NO 1 : 47 CARTONS"</div>';
      return;
    }
    let html =
      '<table class="tbl" style="font-size:12px"><thead><tr><th>Item No</th><th>Qty Karton</th></tr></thead><tbody>';
    parsed.items.forEach(
      (it, i) =>
        (html += `<tr><td><input type="number" value="${it.itemNo}" class="ocr-edit" data-ino="${i}" min="1"></td><td><input type="number" value="${it.qty}" class="ocr-edit" data-iqty="${i}" min="0"></td></tr>`),
    );
    html += "</tbody></table>";
    if (totalMatch)
      html += `<div style="margin-top:4px;color:var(--green);font-size:11px">Total detected: ${totalMatch[1]}</div>`;
    pre.innerHTML = html;
    pre.querySelectorAll("[data-ino]").forEach((el) =>
      el.addEventListener("change", (e) => {
        parsed.items[+e.target.dataset.ino].itemNo = pi(e.target.value) || 1;
      }),
    );
    pre.querySelectorAll("[data-iqty]").forEach((el) =>
      el.addEventListener("change", (e) => {
        parsed.items[+e.target.dataset.iqty].qty = pi(e.target.value);
      }),
    );
  }
  ...
  function parseCfr(text) {
    ...
    if (!parsed.cfr.length) {
      pre.innerHTML =
        '<div class="empty" style="padding:8px;font-size:12px">Tidak terdeteksi.</div>';
      return;
    }
    let html =
      '<table class="tbl" style="font-size:12px"><thead><tr><th>Item</th><th>CFR ($)</th></tr></thead><tbody>';
    parsed.cfr.forEach(
      (v, i) =>
        (html += `<tr><td>Item ${i + 1}</td><td><input type="text" inputmode="decimal" value="${v}" class="ocr-edit" data-cv="${i}"></td></tr>`),
    );
    pre.innerHTML = html + "</tbody></table>";
    pre.querySelectorAll("[data-cv]").forEach((el) =>
      el.addEventListener("change", (e) => {
        parsed.cfr[+e.target.dataset.cv] = pf(e.target.value);
      }),
    );
  }
  ...
  function parseGlobal(text) {
    ...
    setBar(
      "global",
      100,
      `✅ FOB: $${fmt(parsed.global.fob || 0)}, Freight: $${fmt(parsed.global.freight || 0)}`,
    );
  }
  ...
  function applyParsed(zone) {
    if (zone === "measurement") {
      ...
      checks.forEach((cb, i) => {
        ...
        const r = parsed.measurement[i];
        if (!r) return;
        const cbmPU = (r.p * r.l * r.t) / 1_000_000;
        templates.push({
          id: Date.now() + i,
          name: `Dus-${templates.length + 1}`,
          p: r.p,
          l: r.l,
          t: r.t,
          cbmTarget: r.cbm,
          qtyTotal: Math.round(r.cbm / cbmPU),
          cbmPerUnit: cbmPU,
        });
        added++;
      });
      if (added) {
        showAlert(`✅ ${added} ukuran ditambahkan ke Master Template!`);
        renderAll();
      }
    } else if (zone === "items") {
      ...
      showAlert(`✅ ${parsed.items.length} item dimuat!`);
      renderAll();
    } else if (zone === "cfr") {
      parsed.cfr.forEach((val, i) => {
        if (items[i]) {
          items[i].cfrInput = val;
          items[i].fobInput = 0;
          items[i].lastItemEdited = "cfr";
        }
      });
      showAlert(
        `✅ CFR diterapkan ke ${Math.min(parsed.cfr.length, items.length)} item!`,
      );
      renderAll();
    }
  }
  ...
  const tfP = $("tf-p"),
    ...
    tfQty = $("tf-qty"),
    tfCbm = $("tf-cbm"),
    tfPrev = $("tf-cbm-preview");
  let lastEdited = "";

  function getCbmPerUnit() {
    const p = pf(tfP.value),
      l = pf(tfL.value),
      t = pf(tfT.value);
  ...
  function updateFormPreview() {
    ...
    const qty = pi(tfQty.value),
      cbm = pf(tfCbm.value);

    if (cbmPU <= 0) {
      tfPrev.textContent = "Isi P, L, T dulu";
      tfPrev.style.color = "";
      return;
    }
  ...
  tfQty.addEventListener("input", () => {
    lastEdited = "qty";
    updateFormPreview();
  });
  tfCbm.addEventListener("input", () => {
    lastEdited = "cbm";
    updateFormPreview();
  });
  [tfP, tfL, tfT].forEach((el) =>
  ...
  $("btn-add-tpl").addEventListener("click", () => {
    ...
    const qty = pi(tfQty.value),
      cbm = pf(tfCbm.value);
    if (!p || !l || !t) {
      showAlert("Isi P, L, dan T!", "error");
      return;
    }
    if (!qty && !cbm) {
      showAlert("Isi Qty Karton atau CBM Total!", "error");
      return;
    }
    const cbmPU = (p * l * t) / 1_000_000;
    const finalQty = qty || Math.round(cbm / cbmPU);
    const finalCbm =
      lastEdited === "cbm" && cbm > 0 ? cbm : R2(cbmPU * finalQty);
    templates.push({
      id: Date.now(),
      name,
      p,
  ...
  $("tpl-body").addEventListener("click", (e) => {
    ...
    if (btn) {
      templates.splice(+btn.dataset.delTpl, 1);
      renderAll();
    }
  });
  $("tpl-body").addEventListener("change", (e) => {
    ...
    if (t.dataset.tplName !== undefined) {
      templates[+t.dataset.tplName].name =
        t.value.trim() || `Dus ${+t.dataset.tplName + 1}`;
    } else if (t.dataset.tplP !== undefined) {
      const i = +t.dataset.tplP;
      templates[i].p = pf(t.value);
      templates[i].cbmPerUnit =
        (templates[i].p * templates[i].l * templates[i].t) / 1_000_000;
      templates[i].cbmTarget = R2(
        templates[i].cbmPerUnit * templates[i].qtyTotal,
      );
      renderAll();
    } else if (t.dataset.tplL !== undefined) {
      const i = +t.dataset.tplL;
      templates[i].l = pf(t.value);
      templates[i].cbmPerUnit =
        (templates[i].p * templates[i].l * templates[i].t) / 1_000_000;
      templates[i].cbmTarget = R2(
        templates[i].cbmPerUnit * templates[i].qtyTotal,
      ...
      const i = +t.dataset.tplT;
      templates[i].t = pf(t.value);
      templates[i].cbmPerUnit =
        (templates[i].p * templates[i].l * templates[i].t) / 1_000_000;
      templates[i].cbmTarget = R2(
        templates[i].cbmPerUnit * templates[i].qtyTotal,
      ...
      templates[i].qtyTotal = pi(t.value);
      templates[i].cbmTarget = R2(
        templates[i].cbmPerUnit * templates[i].qtyTotal,
      );
      renderAll();
      ...
      templates[i].cbmTarget = pf(t.value);
      if (templates[i].cbmPerUnit > 0)
        templates[i].qtyTotal = Math.round(
          templates[i].cbmTarget / templates[i].cbmPerUnit,
        );
      renderAll();
    }
  });
    ...
    const data = items.map((item) => {
      ...
      item.cartons.forEach((c) => {
        ...
        if (tp) {
          ...
          const cbmPU =
            tp.qtyTotal > 0 ? tp.cbmTarget / tp.qtyTotal : tp.cbmPerUnit;
          rawCbm += cbmPU * c.qty;
          totQty += c.qty;
        }
      });
  ...
  function renderItems() {
    ...
    const cfrFobAll = calcAllCfrFob(freights);
  ...
  $("items-wrap").addEventListener("change", (e) => {
    ...
    if (t.dataset.tq !== undefined) {
      items[+t.dataset.tq].targetQty = pi(t.value);
      renderAll();
    } else if (t.dataset.cfr !== undefined) {
      const i = +t.dataset.cfr;
      items[i].cfrInput = pf(t.value);
      items[i].lastItemEdited = "cfr";
      renderAll();
    } else if (t.dataset.fob !== undefined) {
      const i = +t.dataset.fob;
      items[i].fobInput = pf(t.value);
      items[i].lastItemEdited = "fob";
      renderAll();
    }
  });
  ...
  $("btn-auto-solve").addEventListener("click", async () => {
    if (!templates.length || !items.length) {
      showAlert("Buat Template dan Item dulu!", "error");
      return;
    }
    const totT = items.reduce((s, it) => s + it.targetQty, 0),
      totA = templates.reduce((s, tp) => s + tp.qtyTotal, 0);
    if (!totT) {
      showAlert("Isi Target Karton di setiap Item!", "error");
      return;
    }
    if (
      totT !== totA &&
      !(await showConfirm(`⚠️ Target (${totT}) ≠ Template (${totA}). Lanjut?`))
    )
      return;

    // Group templates by P (panjang) dimension for marking-based allocation
    const groups = new Map(); // P → [{ tplId, avail, tp }]
    templates.forEach((tp) => {
      const key = tp.p;
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push({ tplId: tp.id, avail: tp.qtyTotal, t: tp.t });
    });
    // Within each group: sort by T (height) descending — tallest cartons first
    const groupArr = [...groups.values()];
    groupArr.forEach((pool) => pool.sort((a, b) => b.t - a.t));

    // Sort items by targetQty descending (main items first), preserve original index for restore
    ...
    // Restore original item order (itemNo tidak berubah)
    renderAll();
    const left = groupArr.reduce(
      (s, pool) => s + pool.reduce((s2, sl) => s2 + sl.avail, 0),
      0,
    );
    showAlert(
      left === 0
  ...
  function validate() {
    ...
    // Warning: Rate × Total CBM vs Global Freight
  ...
  $("btn-reset-all").addEventListener("click", async () => {
    if (
      !(await showConfirm(
        "⚠️ Reset semua data? Semua template, item, dan nilai global akan dihapus.",
      ))
    )
      return;
    templates.length = 0;
